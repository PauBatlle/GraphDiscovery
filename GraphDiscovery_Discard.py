...
class GraphDiscovery(object):
    ...
    def discovery_in_graph(self, X, ks, gamma, G, names, examing_nodes, beta1=1e-1, beta2=1e-2, beta3=1e-3, epsilon=1e-3, verbose=False):
        ...
        # modes include two types: feature modes (fmodes) and kernel modes (kmodes)
        # Create feature modes
        fmodes = onp.array([ConstantMode(onp.array([-1]))])
        # Add linear fmodes
        linear_modes = [LinearMode(onp.array([i]), onp.array([names[i]]), beta1) for i in range(d)]
        fmodes = onp.append(fmodes, linear_modes)
        # Add quadratic fmodes
        quadratic_modes = [QuadraticMode(onp.array([i, j]), onp.array([names[i], names[j]]), beta2) for i in range(d) for j in range(i+1)]
        fmodes = onp.append(fmodes, quadratic_modes)
        # The number of fmodes
        fmodes_num = len(fmodes)

        # The correspondence between fmodes and nodes.
        # Entries at (i, j) equal to 1 implies that there is a relation between the ith fmode and the jth node.
        # Otherwise, entries at (i, j) equal to 0 implies there is no relation between the ith fmode and the jth node.
        # The matrix size is (fmodes_num, d + 1), which means we have the constant node.
        correspondence_mtx = onp.zeros((fmodes_num, d+1))
        ...
        # build the covariance matrix for the feature modes
        fKxx = onp.zeros((N, N))
        for mode in fmodes:
            fKxx = fKxx + self.Kn(lambda x, y: mode.kappa(x, y), X.T)

        # build the covariance matrix for the kernel modes
        Ms = {}
        for j in range(d):
            Ms[j] = 1 + ks[j](X[j], X[j])

        kKxx = reduce(np.multiply, list(Ms.values()))
        ...
        fKbs_varphi_varphi = onp.zeros((len(fmodes), r, r))  # Record the matrix Kb(varphi, varphi) for each fmode

        for t in range(len(fmodes)):
            mode = fmodes[t]
            ...
            fKbs_varphi_varphi[t, :, :] = func(alphas, alphas)
        ...
        for i in examing_nodes_indices:
            ...
            modes_i = onp.where(correspondence_mtx[:, i] == 1)[0] # The indices of fmodes related to the ith node

            # The operations below put the fmode beta1 * xi * xi' into Class a, all other fmodes related to the ith
            # node to Class c, and the rest fmodes to Class b
            activation_codes = onp.ones(fmodes_num) # Record the activation of each mode, 0 for Class a, 1 for Class b, -1 for Class c
            ...
            fmodes_b_indices = onp.where(activation_codes == 1)[0] # Choose those fmodes in Class b
            fKbis_varphi_varphi = fKbs_varphi_varphi[fmodes_b_indices, :, :] # Record the matrix Kbi(varphi, varphi) for each activated fmode Kbi in Class b
